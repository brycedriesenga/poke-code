...
import { LocalMcpServer } from "./transport/local-mcp-server.js";
...
export interface AppProps {
  ...
  mcpServerUrl?: string;
  mcpProtocols?: string[];
  mcpLocalServerEnabled?: boolean;
  mcpLocalServerHost?: string;
  mcpLocalServerPort?: number;
  chatId?: number;
  handleId?: number;
  dbPath: string;
...
function App(props: AppProps) {
  ...
  const localMcpServerRef = useRef<LocalMcpServer | null>(null);
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      if (mcpLocalServerEnabled) {
        const localServer = new LocalMcpServer({
          host: mcpLocalServerHost ?? "127.0.0.1",
          port: mcpLocalServerPort ?? 8787,
          apiClient: apiClient.current,
        });
        try {
          await localServer.start();
          localMcpServerRef.current = localServer;
        } catch (err: unknown) {
          const msg = err instanceof Error ? err.message : String(err);
          appendMessage({ role: "system", content: formatErrorWithHint(`Local MCP server failed: ${msg}`) });
        }
      }
    ...
    return () => {
      ...
      if (localMcpServerRef.current) {
        void localMcpServerRef.current.stop();
        localMcpServerRef.current = null;
      }
    };
  }, [transport, mcpServerUrl, mcpProtocols, mcpLocalServerEnabled, mcpLocalServerHost, mcpLocalServerPort, appendMessage]);
