...
describe('parseIntent', () => {
  ...
  describe('read_file intents', () => {
    ...
    it('reads a file after "view"', () => {
      expect(parseIntent('view Makefile', cwd)).toEqual([
        { tool: 'read_file', params: { path: r('Makefile') } },
      ]);
    });
  ...
  describe('list_dir intents', () => {
    ...
    it('lists an extensionless path-like token as list_dir', () => {
      expect(parseIntent('list `components/`', cwd)).toEqual([
        { tool: 'list_dir', params: { path: r('components') } },
      ]);
    });
  ...
  describe('bash intents', () => {
    ...
    it('runs a double-quoted command after "execute"', () => {
      expect(parseIntent('let me execute "ls -la"', cwd)).toEqual([
        { tool: 'bash', params: { command: 'ls -la' } },
      ]);
    });

    it('runs a command after "try"', () => {
      expect(parseIntent('try `make build`', cwd)).toEqual([
        { tool: 'bash', params: { command: 'make build' } },
      ]);
    });
  ...
  describe('glob intents', () => {
    ...
    it('finds files after "search for files"', () => {
      expect(parseIntent('search for files `*.json`', cwd)).toEqual([
        { tool: 'glob', params: { pattern: '*.json' } },
      ]);
    });
  ...
  describe('grep intents', () => {
    ...
    it('searches with a quoted pattern', () => {
      expect(parseIntent('grep for "useEffect"', cwd)).toEqual([
        { tool: 'grep', params: { pattern: 'useEffect' } },
      ]);
    });
  });
  ...
  describe('multiple intents in one message', () => {
    it('extracts read_file and list_dir from a single message', () => {
      const result = parseIntent(
        "I'll read package.json and check the src/ directory",
        cwd,
      );
