...
describe('parseBrackets', () => {
  ...
  describe('[read]', () => {
    ...
    it('parses a nested path', () => {
      ...
      expect(result).toEqual([
        { tool: 'read_file', params: { path: r('src/api/client.ts') } },
      ]);
    });
    ...
    it('parses a read with line range', () => {
      ...
      expect(result).toEqual([
        { tool: 'read_file', params: { path: r('src/app.tsx'), offset: 10, limit: 16 } },
      ]);
    });
  ...
  describe('[run]', () => {
    ...
    it('parses a piped command', () => {
      ...
      expect(result).toEqual([
        { tool: 'bash', params: { command: 'cat package.json | grep scripts' } },
      ]);
    });
  });
  ...
  describe('[find]', () => {
    it('parses a glob pattern', () => {
      ...
      expect(result).toEqual([
        { tool: 'glob', params: { pattern: '**/*.test.ts', path: CWD } },
      ]);
    });
    ...
    it('parses a specific extension glob', () => {
      ...
      expect(result).toEqual([
        { tool: 'glob', params: { pattern: 'src/**/*.tsx', path: CWD } },
      ]);
    });
  });
  ...
  describe('[grep] and [search]', () => {
    it('parses a simple grep', () => {
      ...
      expect(result).toEqual([
        { tool: 'grep', params: { pattern: 'useState', path: CWD } },
      ]);
    });
    ...
    it('parses grep with file filter', () => {
      ...
      expect(result).toEqual([
        { tool: 'grep', params: { pattern: 'handleSend', glob: '*.tsx', path: CWD } },
      ]);
    });
    ...
    it('treats [search] as web_search', () => {
      ...
      expect(result).toEqual([
        { tool: 'web_search', params: { query: 'parseResponse' } },
      ]);
    });
  });
